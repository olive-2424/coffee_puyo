class Player {
  ...
  static initialize() {
    ...
    document.addEventListener('keydown', (e) => {
      // キーボードが押された場合
      // keydown,keyupイベントはどのキーが押されたかを示す値を提供する
      // 上記はデフォルトで押されたいる間、イベントを繰り返す
      switch (e.keyCode) {
        // keyCodeはKeyboardEventインターフェースの読み取り専用プロパティで押されたキーの値を取得する
        // ただし、現在は非推奨で代わりにKeyboard.Event.codeやkeyを使用した方がいいらしい
        case 37: // 左向きキー
          this.keyStatus.left = true;
          e.preventDefault();
          // preventDefault()はEventインターフェースのメソッド
          // イベントのデフォルトの処理をキャンセルする
          // 今回はkeydownイベントをキャンセルしている
          return false;
        // ある要素で発火したイベントはその全ての親要素に伝播する(バブリング)
        // jqueryでは親要素へのイベント伝播を止める、javascriptでは止めない挙動になる
        // 今回の場合、keydownイベントは行われるがそのイベントのデフォルト処理は行われないようにできる
        case 38: // 上向きキー
  ...
  static createNewPuyo() {
    ...
    if (Stage.board[0][2]) {
      // 空白でない場合は新しいぷよを置けない(本来×印がある場所)
      return false;
    }
    // 新しいぷよの色を決める
    const puyoColors = Math.max(
      1,
      Math.min(5, window.localStorage.getItem('level'))
    );
    // Math.max関数は引数に渡された値で最大の数を返す、引数がない場合はInfinityを返す
    // Math.min関数は引数に渡された値で最初の数を返す
    this.centerPuyo = Math.floor(Math.random() * puyoColors) + 1;
    // Math.floor()関数は引数に与えられた数値以下の最大の整数を返す
    // Math.random()関数は０以上１未満の小数をランダムで返す
    // 上記二つを使ってランダムに取得したgetPuyoに渡すことでランダムにぷよの色を取得できる
    ...
    this.puyoStatus = {
      x: 2, // 中心ぷよの位置: 左から2列目
      y: -1, // 画面上部ギリギリから出てくる
      left: 2 * Config.puyoImgWidth, // 2 *  40
      // left はCSSのプロパティで位置指定要素の水平位置の決定に関与する（意味合いがある
      // left　はぷよが作成されて、初期の水平位置を指定するためのプロパティ
      top: -1 * Config.puyoImgHeight, // -1 * 40
      // top はCSSのプロパティで位置指定要素の垂直位置の決定に関与する（意味合いがある
      // top　はぷよが作成されて、初期の垂直位置を指定するためのプロパティ
    ...
    return true;
    // 上記で使われているthisはcreateNewPuyo()を呼び出したインスタンス自身を示す
    // thisは関数の外側では
    // オブジェクト内でthisを使うとそのオブジェクト自身を示す
  }
  ...
  static setPuyoPosition() {
    ...
    // 上記二つで中心ぷよの現在の水平(style.left)、垂直位置(style.top)をpxで設定する
    const x =
      this.puyoStatus.left +
      Math.cos((this.puyoStatus.rotation * Math.PI) / 180) *
        Config.puyoImgWidth;
    // Math.cos()は静的関数で、指定された角度のコサインを返す
    const y =
      this.puyoStatus.top -
      Math.sin((this.puyoStatus.rotation * Math.PI) / 180) *
        Config.puyoImgHeight;
    // Math.sin()関数は、引数として与えた数のサインを返す
    this.movablePuyoElement.style.left = x + 'px'; // 126px
    this.movablePuyoElement.style.top = y + 'px'; // -86.4000...px
    // 上記二つで落ちるときの上にあるぷよの水平、垂直位置をpxで設定する
    // これにより新しく作成されて落ちてくる２つのぷよの初期位置をウィンドウのサイズなどに合わせて設定している
  }
